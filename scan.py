...
-----------------------------------------
- Auto-launches in a new terminal window (via batch/shell script).
- Stunning hacker-style UI with dynamic ASCII art, glitch effects, neon colors, and matrix rain.
...
- Glitchy cyberpunk animations for immersive experience.
...
class AnimatedScanner:
    def __init__(self, title="Cyber Malware Scanner", paths=None):
        ...
        self._spinner_cycle = cycle(["[HACKING]", "[INFILTRATING]", "[DECODING]", "[BREACHING]", "[CRACKING]"])
        ...
        self.elapsed = 0.0
        self.checked = 0
        self.fake_speed = random.uniform(800, 2000)  # Higher speed for realism
        self.progress = 0.0
    ...
    def _ui_loop(self, external_proc=None):
        ...
        while not self._stop_event.is_set():
            ...
            # Progress logic: Slow to 95%, then fast to 100%
            if running:
                inc = random.uniform(0.5, 2.5) * (1.0 - (self.progress / 100.0))
                self.progress = min(self.progress + inc, 95.0)
            else:
                inc = random.uniform(8, 20)
            ...
            # Glitch effect (occasional random chars)
            self.glitch_counter += 1
            glitch = random.choice(["~", "#", "%", "&"]) if self.glitch_counter % 20 == 0 else ""
            
            # Build UI
            lines = []
            lines.append(Fore.CYAN + Style.BRIGHT + "╔" + "═" * 78 + "╗")
            lines.append(Fore.CYAN + f"║ {self.title.center(76)} {glitch}║")
            lines.append(Fore.CYAN + "╠" + "═" * 78 + "╣")
            ...
            lines.append(Fore.BLUE + self._render_line(f"Data Stream: {binary * 8}", f"Hex: {hex(random.randint(0, 0xFFFF))[2:].upper()}"))
            ...
            if not running and self.progress >= 99.9:
                ...
                break
            time.sleep(0.07)  # Smoother animation
...
def interactive_menu(detections):
    """Interactive menu for handling detections."""
    if not detections:
        print(Fore.GREEN + "[+] System secure. No intrusions detected.")
        return
    qdir = ensure_quarantine_dir()
    print(Fore.RED + "[!] Intrusions detected in the matrix:")
    for i, (p, sig) in enumerate(detections, start=1):
        print(Fore.RED + f"[{i}] Target: {p}  -- Signature: {sig}")
    while True:
        choice = input(Fore.CYAN + "Select target (or 'all', 'exit' to abort): ").strip().lower()
        if choice in ("exit", "q"):
            break
        targets = []
        if choice == "all":
        ...
        for idx in targets:
            if idx < 1 or idx > len(detections):
                print(Fore.YELLOW + "Target out of range.")
                continue
            file_path, sig = detections[idx - 1]
            print(Fore.MAGENTA + f"\nTarget File: {file_path}")
            print(Fore.RED + f"Threat Sig: {sig}")
            print(Fore.CYAN + "1) Isolate (Quarantine)  2) Exile (Recycle)  3) Eradicate (Delete)  4) Ignore")
            act = input(Fore.CYAN + "Execute command: ").strip()
            if act == "1":
                ok, msg = quarantine_file(file_path, qdir)
                print(Fore.GREEN if ok else Fore.RED + f"Isolation: {msg}")
            elif act == "2":
                ok, msg = send_to_recycle_or_fallback(file_path)
                print(Fore.GREEN if ok else Fore.RED + f"Exile: {msg}")
            elif act == "3":
                c = input(Fore.RED + "Confirm eradication (type 'CONFIRM'): ").strip().upper()
                if c == "CONFIRM":
                    ok, msg = delete_permanent(file_path)
                    print(Fore.GREEN if ok else Fore.RED + f"Eradication: {msg}")
                else:
                    print(Fore.YELLOW + "Operation aborted.")
            else:
                print(Fore.YELLOW + "Target ignored.")

# ---------------------------
# Cyber Intro & Welcome
# ---------------------------
def cyber_intro(system_info):
    """Hacker-style intro with matrix rain and dynamic typing."""
    os.system("cls" if os.name == "nt" else "clear")
    print(Fore.GREEN + Style.BRIGHT + "Initializing Cyber Matrix Protocol...")
    time.sleep(1.5)
    
    # Matrix Rain Effect
    matrix_lines = [" " * random.randint(0, 80) + random.choice("01") for _ in range(25)]
    for _ in range(15):
        sys.stdout.write("\x1b[2J\x1b[H")
    ...
    welcome_msg = f"Welcome to the Matrix, {device_name}. Cyber Defense Systems Online."
    print(Fore.CYAN + "═" * 80)
    ...
    print(Fore.YELLOW + "Deploying Cyber Arsenal...")
...
def main():
    ...
    # Scan Mode Selection
    print(Fore.MAGENTA + "═[ SELECT CYBER SCAN MODE ]═")
    ...
    choice = input(Fore.YELLOW + "Execute Mode [1-3]: ").strip()
    
    if choice == "3":
        path = input(Fore.CYAN + "Enter Target Path: ").strip()
        if not os.path.exists(path):
            print(Fore.RED + "Path not in matrix. Falling back to Quick Scan.")
        ...
        if platform.system() == "Windows":
            ...
            paths = ["/"]
        print(Fore.CYAN + f"Full Matrix Scan: Targeting {', '.join(paths)}")
    else:
    ...
    try:
        if clam:
            ...
            try:
                ...
                print(Fore.RED + "[!] ClamAV activation failed:", e)
                proc = None
        ...
        ui._stop_event.set()
        print(Fore.YELLOW + "\n[!] Mission aborted. Declassifying partial intel.")
    except Exception as e:
        ui._stop_event.set()
        print(Fore.RED + "[!] Error in matrix:", e)
    ...
    if not infected:
        ...
        for i, (p, sig) in enumerate(infected, start=1):
            ...
            print(Fore.RED + f"║ [{i}] Sig: {sig} ".ljust(76) + " ║")
    ...
    print(Fore.CYAN + "\n[*] Matrix Session Terminated at " + datetime.now().strftime("%H:%M:%S") + ". Stay Vigilant! 🔒")